...
- **Webhook traffic is tied to one merchant's Razorpay credentials** — there is still no per-merchant Razorpay OAuth/Connect integration; every merchant in a given deployment shares one `RAZORPAY_KEY_ID`/webhook secret pair. **Updated**: within that shared account, payment-to-merchant *attribution* is now correct for the flow this app actually drives payments through. `app/api/checkout/create-order/route.js` stashes the logged-in merchant's ID in the Razorpay Order's `notes` at creation time; `app/api/webhooks/razorpay/route.js`'s `resolveMerchantId()` fetches the order back by `payment.order_id` and reads that note, falling back to `DEFAULT_MERCHANT_ID` only if it's missing/malformed or the noted merchant no longer exists (pending end-to-end verification against a real payment). The residual gap: anything that doesn't originate from our in-app checkout — a Payment Link or QR code created directly in Razorpay's own dashboard, for example — carries no such note and still falls back to `DEFAULT_MERCHANT_ID`, since there's no other way to know which merchant it belongs to without real per-merchant Razorpay accounts.
