...
import { withMerchantLock, LockAcquisitionError } from "@/lib/merchantLock";
...
export async function ingestTransaction(event) {
  ...
  // Measured around the whole scoreTransaction() call, including any
  // internal Groq-primary -> Groq-secondary -> Gemini failover it does -
  // this is "how long did it take to get a usable score back," from the
  // caller's perspective, not a per-tier breakdown. Not measured on the
  // forceFallback demo path (no real call was made) or when scoreTransaction
  // throws (that duration is real but not attributable to a successful
  // provider, so it's left null rather than recorded against "fallback").
  let scoringLatencyMs = null;
  if (forceFallback) {
    ...
    try {
      ...
      scoringLatencyMs = Date.now() - scoringStart;
    } catch (err) {
      usedFallback = true;
  ...
  // scoringOutput.provider ("groq-primary" | "groq-secondary" | "gemini") is
  // only present when an AI tier actually succeeded - fallbackScore()
  // doesn't set one, so "fallback" is recorded explicitly rather than left
  // null, making it a real, countable bucket alongside the AI tiers.
  const provider = usedFallback ? "fallback" : scoringOutput.provider;
  ...
  // that decision, all under a per-merchant distributed lock (Redis) -
  // otherwise two concurrent requests could both read the same stale total
  // and both get approved past the daily cap. See lib/merchantLock.js.
  //
  // forceNoAutoRefund exists for exactly one reason: the lock is what
  // guarantees the daily-budget read is safe to act on. If the lock can't
  // be acquired at all (Redis unreachable, or genuine contention timeout -
  // see lib/merchantLock.js), we no longer have that guarantee, so
  // auto_refund - the one decision that spends shared budget - can never
  // be considered safe to approve. Everything else about scoring/logging
  // this transaction still happens normally; only that one decision gets
  // downgraded, with the downgrade itself recorded on the audit trail.
  async function runCriticalSection(dailyAuthorizedTotal, forceNoAutoRefund) {
    const settings = await getMerchantSettings(merchantId);
    let policyResult = applyPolicy(scoringOutput, amount, dailyAuthorizedTotal, settings);

    if (forceNoAutoRefund && policyResult.decision === "auto_refund") {
      policyResult = {
        decision: "hold_for_review",
        reason: `downgraded from auto_refund: merchant lock unavailable, so the daily refund budget could not be safely verified under concurrency (original: ${policyResult.reason})`,
      };
    }
  ...
  // occurred on, so replaying/seeding historical data enforces the cap per
  // simulated day rather than against the real wall-clock date. Scoped to
  // source: razorpay_live only - it's a real-money budget, so synthetic
  ...
  // confirmed successful Razorpay refunds (refundExecuted) - a failed real
  // refund still consumes budget. That's intentional: it prevents retrying
  // a failed refund from opening up budget for an immediate second attempt
  // that pushes the day's total past the merchant's cap.
  async function readDailyAuthorizedTotal() {
    ...
    return dailyAgg._sum.amount || 0;
  }

  let saved, policyResult, settings;
  try {
    ({ saved, policyResult, settings } = await withMerchantLock(merchantId, async () => {
      const dailyAuthorizedTotal = await readDailyAuthorizedTotal();
      return runCriticalSection(dailyAuthorizedTotal, false);
    }));
  } catch (err) {
    // Only a failure to acquire the lock itself gets the degraded-safe
    // fallback below. Anything else (a real DB error, a bug inside
    // runCriticalSection) propagates as-is - re-running runCriticalSection
    // in that case would risk masking a real failure as a lock outage and
    // attempting a second, possibly conflicting write.
    if (!(err instanceof LockAcquisitionError)) {
      throw err;
    }
    console.error(
      `Merchant lock unavailable for ${merchantId} (${err.message}) - scoring and saving this transaction anyway, with auto_refund disabled since the daily budget can't be safely verified without the lock.`
    );
    ({ saved, policyResult, settings } = await runCriticalSection(0, true));
  }
