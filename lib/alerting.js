...
export async function sendAlert(transaction, settings) {
  // reasons[0] is often the generic "AI scoring unavailable" fallback
  // notice rather than an actual signal - skip boilerplate lines so the
  // alert leads with something specific to this transaction.
  const topReason =
    (transaction.reasons || []).find(
      (r) => !r.startsWith("⚠️") && !r.startsWith("Fallback recommended") && !r.startsWith("AI recommended")
    ) || "no reason recorded";
